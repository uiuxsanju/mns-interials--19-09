    ...
    <section id="gallery" className="container-x py-20 lg:py-28">
      {heading && (
        <SectionHeading
          ...
          desc="Real projects, photographed after handover. Filter by room to find a direction you like."
        />
      )}
      ...
      <div className="mt-8 columns-1 gap-5 sm:columns-2 lg:columns-3 [&>*]:mb-5">
        {items.map((item, i) => (
          <button
            key={`${item.title}-${i}`}
            onClick={() => setIndex(i)}
            className="card-media group block w-full break-inside-avoid overflow-hidden rounded-2xl bg-card text-left shadow-card transition-shadow hover:shadow-lift"
          >
            <img
              src={item.image}
              alt={item.title}
